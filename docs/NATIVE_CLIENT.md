...
The helper builds locked sources, creates a disposable certificate/key, starts a real host on OS-selected ports, waits for readiness, connects the client and reads back a rendered GPU frame. With `--window`, it then opens a native game window for 120 frames. It stops its processes and removes temporary credentials on success or failure.
...
For publicly trusted server certificates, omit `--certificate`; system certificate trust is then used. The client never disables certificate verification. `--smoke` connects and verifies a single offscreen GPU frame; `--frames N` limits a window run for lifecycle checks. Invalid configuration, incompatible tick rates/content, and failed connections exit nonzero.

## Current limits

- This is a connected gameplay/graphics slice, not a production account system. Sessions are anonymous; reconnect UI, account/character binding and live zone handoff are pending.
